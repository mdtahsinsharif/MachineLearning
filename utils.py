...
def get_data_batches(num_per_cut, batch_size):
	cut_data = reservoir_sample(read_stmd("cut"), num_per_cut)
	wasp_data = reservoir_sample(read_stmd("bee_sting"), num_per_cut)
	return split_and_batch_data(
		cut_data + wasp_data,
		[0]*num_per_cut + [1]*num_per_cut,
		batch_size, 2)
